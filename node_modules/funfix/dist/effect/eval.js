...
export class Eval {
    ...
    transform(failure, success) {
        return this.transformWith(e => Eval.now(failure(e)), a => Eval.now(success(a)));
    }
    /**
     * Creates a new `Eval` by applying the 'success' function to the
     * successful result of the source, or the 'error' function to the
     ...
     */
    static raise(e) { return new Raise(e); }
    /**
...
class Now extends Eval {
    ...
    run() { return Try.success(this.value); }
...
class Raise extends Eval {
    ...
    run() { return Try.failure(this.error); }
    toString() { return `Eval.raise(${JSON.stringify(this.error)})`; }
}
/**
...
function _findErrorHandler(bFirst, bRest) {
    let cursor = bFirst;
    while (true) {
        if (cursor && typeof cursor.onFailure === "function")
            return cursor;
        if (bRest && bRest.length > 0)
            cursor = bRest.pop();
        else
...
function evalRunLoop(start) {
    ...
    while (true) {
        if (current instanceof Now) {
            ...
            if (!bind)
                return Try.success(current.value);
            bFirst = null;
            try {
            ...
            if (!bind)
                return Try.failure(current.error);
            bFirst = null;
            try {
